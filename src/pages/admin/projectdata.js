...
const Projectdata = () => {
 ...
 <h1 className="restrictedbodyh1">Restricted Web Page</h1>
  ...
  const [aiartdata, setAiartData] = useState([]);
  ...
  const deleteImage = (id) => {
    ...
    if (isOkay) {
      fetch(process.env.NEXT_PUBLIC_API_URL+`/image/${id}`, { method: "delete" })
        .then(() => alert("done"))
        .then(() => {
          if (refresh == true) {
            setRefresh(false)
  ...
  const deleteAdv = async (id) => {
    const isOkay = confirm("Are you sure to delete it?");
    if (isOkay) {
      fetch(process.env.NEXT_PUBLIC_API_URL+`/adv/${id}`, { method: "delete" })
        .then(() => alert("done"))
        .then(() => {
          if (refresh == true) {
            setRefresh(false)
    ...
    <>
      ...
      <div className="mainbody">
        ...
        <div className="digitalartdiv" id="digitalart-input-div">
         ...
         {/* aiarts project file */}
         <div className="aiartdiv" id="aiart-input-div">
          <div
            style={{
              display: "flex",
              flexDirection: "column",
          ...
          >
            {contactdata.map((item) => (
              ...
              >
                ...
                <div style={{ display: "flex" }}>
                  <p style={{ marginRight: "20px" }}>Design for:</p>
                  <p style={{ fontWeight: "600" }}>{item.designfor}</p>
                </div>
                <div style={{ display: "flex" }}>
